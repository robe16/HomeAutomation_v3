...
class InfoService:
    ...
    # def __init__(self, type, q_dvc, queues):
    ...
    #                 True
    #     print_msg('Thread stopped: {type}'.format(type=self._type), dvc_or_acc_id='')
    ...
    def _getHtml_generic(self, args):
        return urlopen('web/html/html_info_services/' + get_device_html_command(self._type)).read().encode('utf-8').format(**args)
    ...
    def getHtml_settings_generic(self, args):
        ...
        if html_file:
            return urlopen('web/html/html_info_services/' + html_file).read().encode('utf-8').format(**args)
        else:
            return ''